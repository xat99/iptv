...
// Automatikusan levágjuk az IPTV_URL végéről a felesleges perjeleket, ha lennének
...
// ÚJ: Az app.all() elfogadja a Smarters Pro GET és POST kéréseit is!
app.all(['/player_api.php', '/xmltv.php', '/panel_api.php'], async (req, res) => {
    // A Smarters Pro néha a query-ben, néha a body-ban küldi az adatot
    const username = req.query.username || req.query.user;
    const password = req.query.password || req.query.pass;
    ...
    try {
        ...
        const fetchOptions = {
            method: 'GET', // Az eredeti szerver felé mindig GET-ként küldjük
            headers: {
...
app.get('/:type/:user/:pass/:filename', (req, res) => {
    ...
    const redirectUrl = `${cleanIptvUrl}/${type}/${IPTV_USER}/${IPTV_PASS}/${filename}`;
